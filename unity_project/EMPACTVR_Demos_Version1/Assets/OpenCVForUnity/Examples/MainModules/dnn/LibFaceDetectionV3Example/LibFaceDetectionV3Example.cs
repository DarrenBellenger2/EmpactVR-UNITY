...
namespace OpenCVForUnityExample
{
    ...
    public class LibFaceDetectionV3Example : DnnObjectDetectionWebCamTextureExample
    {
        ...
        public override void OnWebCamTextureToMatHelperInitialized()
        {
            ...
            grayMat = new Mat(640, 480, CvType.CV_8UC1);			
			faces = new MatOfRect();			
        ...
        protected override void postprocess(Mat frame, List<Mat> outs, Net net, int backend = Dnn.DNN_BACKEND_OPENCV)
        {
            ...
            for (int i = 0; i < indices.total(); ++i)
            {
				...
				// Create faces rect from bboxes

				Imgproc.cvtColor(frame, grayMat, Imgproc.COLOR_RGBA2GRAY);
				Imgproc.equalizeHist(grayMat, grayMat);				
				cascade.detectMultiScale(grayMat, faces, 1.05, 6, 2, // TODO: objdetect.CV_HAAR_SCALE_IMAGE
					new Size(grayMat.cols() * 0.2, grayMat.rows() * 0.2), new Size());
				
				List<MatOfPoint2f> landmarks = new List<MatOfPoint2f>();
	
				
				faces_copy = new MatOfRect( new OpenCVForUnity.CoreModule.Rect((int)bbox_arr[0],(int)bbox_arr[1],(int)bbox_arr[2],(int)bbox_arr[3]));	
				//OpenCVForUnity.CoreModule.Rect[] rects = faces.toArray ();
				//for (int k = 0; k < rects.Length; k++) {
				//	Debug.Log ("detect faces " + rects [k]);
				//}

				
				
				
				facemark.fit(frame, faces_copy, landmarks);				
				for (int j = 0; j < landmarks.Count; j++)
				{
					MatOfPoint2f lm = landmarks[0];
					float[] lm_float = new float[lm.total() * lm.channels()];
					MatUtils.copyFromMat<float>(lm, lm_float);

					points = ConvertArrayToVectorList(lm_float);
				...
				if (points != null)
				{
					...
					for (int j = 0; j < points.Count; j++) { Imgproc.circle(frame, new Point(points[j].x, points[j].y), 1, new Scalar(255, 0, 0, 255), 1); }
				}
        ...
        private List<Vector2> ConvertArrayToVectorList(float[] arr, List<Vector2> pts = null)
        {
            if (pts == null)
            {
